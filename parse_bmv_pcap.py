...
def parse_bmv_mensaje_P(bytes_array):
    ...
    msg_P = {"numeroInstrumento": parse_bmv_int32(bytes_array[1:5]),
             ...
             "precio": parse_bmv_precio8(bytes_array[17:25]), "tipoConcertacion": parse_bmv_alfa(bytes_array[25:26]),
             "folioHecho": parse_bmv_int32(bytes_array[26:30]), "fijaPrecio": parse_bmv_alfa(bytes_array[30:31]),
             "tipoOperacion": parse_bmv_alfa(bytes_array[31:32]), "importe": parse_bmv_precio8(bytes_array[32:40]),
             "compra": parse_bmv_alfa(bytes_array[40:45]), "vende": parse_bmv_alfa(bytes_array[45:50]),
             "liquidacion": parse_bmv_alfa(bytes_array[50:51]), "subasta": parse_bmv_alfa(bytes_array[51:52])}
    return msg_P
...
def parse_bmv_udp_packet(packet):
    ...
    assert longitud == len(packet.data), "Longitud $(longitud) must be equal to the packet size $(len(packet.data))"
